...
function updateMonthHeaders() {
    ...
    if (subRow) {
        ...
        const buildGroup = (groupClass) => {
            ...
            periods.forEach((p, i) => {
                const cls = (i === n - 1) ? 'class="group-divider"' : '';
                html += `<th ${cls}>${p.display}</th>`;
            });
        };
...
// Render metric cell
function createMetricCell(current, previous, isLast = false) {
    ...
    return `
        <td class="metric-cell ${dividerClass}">
...
function createHealthScoreCell(score, prevScore, isLast = false) {
    const dividerClass = isLast ? 'group-divider' : '';

    // Handle 'Not Started' / Implantation state
    if (score === null) {
        return `
            <td class="metric-cell usage-cell ${dividerClass}">
    ...
    try {
        ...
        sortedData.forEach((client, index) => {
            ...
            if (visibleColumns.has('group-scale')) {
                metrics.forEach((m, i) => rowHtml += createMetricCell(m.scale[1], m.scale[0], i === n - 1));
            }
            // Group Trips
            if (visibleColumns.has('group-trip')) {
                metrics.forEach((m, i) => rowHtml += createMetricCell(m.trips[1], m.trips[0], i === n - 1));
            }
            // Group Usage
            if (visibleColumns.has('group-usage')) {
                metrics.forEach((m, i) => rowHtml += createHealthScoreCell(m.usage[1], m.usage[0], i === n - 1));
            }
            // Group Crew
            if (visibleColumns.has('group-crew')) {
                metrics.forEach((m, i) => rowHtml += createMetricCell(m.crew[1], m.crew[0], i === n - 1));
            }
            // Group Vehicles
            if (visibleColumns.has('group-veh')) {
                metrics.forEach((m, i) => rowHtml += createMetricCell(m.vehicles[1], m.vehicles[0], i === n - 1));
            }
        ...
        options: {
            ...
            scales: {
                y: {
                    ...
                    grid: { color: '#e5e7eb' },
                    ticks: {
                        stepSize: 20, // Clean intervals: 0, 20, 40, 60, 80, 100
                        color: '#6b7280',
                        callback: v => v + '%',
